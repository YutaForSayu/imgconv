...
func normalizeServer(rawURL string) (string, error) {
	...
	// Replace server<N>- prefix with s<N>-
	host := u.Hostname()
	port := u.Port()
	newHost := host
...
func convertImage(fileData []byte, origName, to string) (FileResult, error) {
	...
	return FileResult{URL: dlURL}, nil
}
...
func main() {
	...
	// Rate limiter: 25 req per 15 detik per IP
	...
	app.Get("/", func(c *fiber.Ctx) error {
		return c.JSON(fiber.Map{
			...
			"version": "1.0.0",
			"routes": fiber.Map{
				"POST /convert":  "Convert gambar (form-data: file, to)",
				"GET  /formats":  "Daftar format yang didukung",
			},
		})
	})
